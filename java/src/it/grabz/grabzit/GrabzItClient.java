...
public class GrabzItClient {
    ...
    public void SetImageOptions(String url, String customId, int browserWidth, int browserHeight, int outputWidth, int outputHeight, ImageFormat format, int delay, String targetElement, boolean requestMobileVersion, String customWaterMarkId) throws UnsupportedEncodingException
    {
        this.request = String.format("%stakepicture.ashx?url=%s&key=%s&width=%s&height=%s&bwidth=%s&bheight=%s&format=%s&customid=%s&delay=%s&target=%s&customwatermarkid=%s&requestmobileversion=%s&callback=",
                                                              ...
                                                              browserWidth, browserHeight, format, encode(customId), delay, encode(targetElement), encode(customWaterMarkId), toInt(requestMobileVersion));
        this.signaturePartOne = applicationSecret + "|" + url + "|";
        this.signaturePartTwo = "|" + format + "|" + outputHeight + "|" + outputWidth + "|" + browserHeight + "|" + browserWidth + "|" + customId + "|" + delay + "|" + targetElement + "|" + customWaterMarkId + "|" + toInt(requestMobileVersion);
    }
    ...
    {
        SetPDFOptions(url, customId, true, PageSize.A4, PageOrientation.PORTRAIT, true, false, "", "", 10, 10, 10, 10, 0, true, "");
    }
    ...
    {
        SetPDFOptions(url, "", true, PageSize.A4, PageOrientation.PORTRAIT, true, false, "", "", 10, 10, 10, 10, 0, true, "");
    }
