...
function checkForResults() {
    ...
    $.getJSON('Home/Results?r=' + Math.floor((Math.random() * 100000) + 1), function (data) {
        $.each(data, function (key, val) {
            if (val.indexOf(".pdf") !== -1) {
                $('#divResults').append('<a title="Click to open" target="_blank" href="' + val + '"><img class="result" src="Content/pdf.png"></img></a>');
            }
            else if (val.indexOf(".docx") !== -1) {
                $('#divResults').append('<a title="Click to open" target="_blank" href="' + val + '"><img class="result" src="Content/docx.png"></img></a>');
            }
            else {
