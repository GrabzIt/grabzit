...
function checkForResults() {
	...
	$.getJSON('ajax/results?r='+Math.floor((Math.random()*100000)+1), function(data) {
		$.each(data, function(key, val) {
			...
			{
				$('#divResults').append('<a title="Click to open" target="_blank" href="'+val+'"><img class="result" src="images/pdf.png"></img></a>');
			}
			else if (val.indexOf(".docx") !== -1)
			{
				$('#divResults').append('<a title="Click to open" target="_blank" href="'+val+'"><img class="result" src="images/docx.png"></img></a>');
			}	
			else
			{
