...
function GrabzItHTMLReducer()
{
	...
	{	
		...
		{		
			...
			if (a.hasAttribute('grabzit-value'))
			{
				b.value = a.getAttribute('grabzit-value');
			}

			if (a.hasAttribute('grabzit-checked'))
			{
				b.checked = true;
			}
	...
	{
		if (a == null || a.nodeType == 3 || a.getAttribute('id') == 'grabzit-shadow' || b == null || b.nodeType == 3 || b.getAttribute('id') == 'grabzit-shadow')
		{
			return;
		}
		...
		{	
			...
			that.setNodeChecked(a, b);
			if (typeof b.attributes != 'undefined')
			{
				var names = new Array();
				for(var i = 0;i < b.attributes.length;i++)
				{
					var name = b.attributes[i].nodeName;
					if (name.indexOf('grabzit-') == -1)
					{
						names.push(name);
					}
				}
		...
		{	
			...
			if (typeof b.attributes != 'undefined' && typeof a.attributes != 'undefined')
			{
				...
				for(var j = 0;j < b.attributes.length;j++)
				{
					var name = b.attributes[j].nodeName;					
					if (name.indexOf('grabzit-') == -1)
					{										
		...
		else
		{
			if (b.parentNode == null || !that.containsNode(b.parentNode.childNodes, a))
			{
				if (b.parentNode == null && previousNode != null)
				{
					alert(previousNode.parentNode.tagName);
				}
				if (a.nodeType != 3)
				{
					var add = a.cloneNode(true);
	...
	this.setNodeValue = function(a, b)
	{
		...
		{			
			if (a.value != b.value)
			{				
				b.setAttribute("grabzit-value", a.value);
			}
		}
	};
	this.setNodeChecked = function(a, b)
	{	
		if (a.getAttribute('type') == 'checkbox' && b.getAttribute('type') == 'checkbox')
		{					
			if (a.checked != b.checked)
			{					
				b.setAttribute("grabzit-checked", a.checked);
			}
		}
	};	
...
function GrabzItWebRecorder()
{
	...
	this.encodeChildren = function(container)
	{
		...
		for(var i = 0;i < container.childNodes.length;i++)
		{
			...
			if (node.tagName != 'TEXTAREA' && node.childNodes.length > 0)
			{
			...
			if (node.hasAttribute('grabzit-value'))
			{				
				encoded += 'v=' + encodeURIComponent(node.getAttribute('grabzit-value')) + '|';
				node.removeAttribute("grabzit-value");
			}

			if (node.firstChild != null && node.firstChild.nodeValue != null && node.firstChild.nodeValue.replace(/(\r\n|\n|\r)/gm,"") != '' && !node.hasAttribute('grabzit-empty'))
			{
			...
			for(var j = 0;j < node.attributes.length;j++)
			{
				...
				encoded = this.encodeAttribute(name, value, encoded);
			}
	...
	{
		if (name == 'grabzit-empty')
		{
			return encoded += 'E=';
		}
		if (name == 'grabzit-checked')
		{
			return encoded += 'g=';
		}		
		if (name == 'type')
		...
		{
			encoded += 'h' + '=' + this.encodeUrl(value);
		}
		else if (name == 'src')
		{
			encoded += 's' + '=' + this.encodeUrl(value);
		}		
		else if (name == 'style')
		{
			encoded += 'S' + '=' + this.encodeUrl(value);
		...
		{
			encoded += 'T' + '=' + this.encodeUrl(value);
		}				
		else if (name == 'class')
		{
	...
	this.decodeAttribute = function(attr)
	{
		...
		if (attr == 'E')
		{
			return 'grabzit-empty';
		}		
		if (attr == 'g')
		{
			return 'grabzit-checked';
		}				
		if (attr == 'v')
		{
			return 'grabzit-value';
		}
		if (attr == 'm')
		{
			return 'method';
	...
	this.decodeChildren = function(parts, limit, diving, that)
	{
		...
		while(that.decodePointer < parts.length)
		{
			...
			if (index > -1)
			{
				...
				if ((tagName == 'textarea' && attr == 'grabzit-value') || attr == 'C')
				{
					//it's content!
