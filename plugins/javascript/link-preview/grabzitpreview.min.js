...
that.elem.appendChild(that._handlePost(JSON.parse(this.responseText)))}};xhttp.open("POST",this._getBaseWebServiceUrl(!0),!0);xhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");xhttp.send(qs)};this._getRootURL=function(isPost)
{if(this.protocol==null)
{this.protocol='//';if(window.location.protocol!='https:'&&window.location.protocol!='http:')
{this.protocol='http://'}}
if(isPost)
{return this.protocol+'grabz.it/services/'}
return this.protocol+'api.grabz.it/services/'};this._getBaseWebServiceUrl=function(isPost)
{return this._getRootURL(isPost)+'javascript.ashx'};this._createQueryString=function(sKey,sValue)
...
return this._createScriptNode(this._getBaseWebServiceUrl(!1)+'?'+this._createQueryString('id',obj.ID))}};this._createErrorMessage=function(error,code){var message=document.createElement('span');message.innerHTML='<strong>GrabzIt Error:</strong> '+error;if(this.options.errorid!=null)
...
reader.readAsDataURL(this.response)}};xhttp.open("GET",that._getRootURL(!1)+'getjspicture.ashx?id='+id,!0);xhttp.responseType="blob";xhttp.send()}
...
var scriptNode=this._createScriptNode(this._getBaseWebServiceUrl(!1)+'?'+this._createQueryString(this.dataKey,this.data));if(insert)
{try
...
{var width=197;var height=154;var onFinishName=null;var onErrorName=null;if(options!=null)
{options=GrabzIt()._cleanOptions(options);if(options.width!=null)
{width=options.width}
...
{children[j].style.display="block"}}}
if(onErrorName!=null)
{var errorFunc=new Function(onErrorName+"('"+message+"',"+code+")");errorFunc()}};try
{GrabzIt(key).ConvertURL(href,options).AddTo(divFrame)}
catch(e)
{alert(e)}
div.appendChild(divFrame);var divCaption=document.createElement('div');divCaption.innerHTML=captionText;divCaption.className="grabzit-preview-caption";div.appendChild(divCaption);link.parentNode.insertBefore(div,link.nextSibling);link.setAttribute('onmouseout',that._getJSFunction(link.getAttribute('onmouseout'))+"document.getElementById('"+guid+"').style.display='none';");link.setAttribute('onmouseover',that._getJSFunction(link.getAttribute('onmouseover'))+"document.getElementById('"+guid+"').style.display='block';")}}(this)}