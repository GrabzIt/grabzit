	...
	return new (function(key)
	{
		...
		this._createQueryString = function(sKey, sValue)
		{
			...
			for(var k in this.options)
			{
				...
				k != 'encryption' && k != 'post' && k != 'noads' && k != 'tvars' && k != 'proxy' && k != 'mergeid' && k != 'address' && k != 'nonotify' && k != 'cachelength' && k != 'onstart' && k != 'hd' && k != 'media')
				{
...
function GrabzItSaveAsPDF(key, options)
{
	...
	this.clicked = function(oObj)
	{
		...
		var functionNameFinish = null;
		var functionNameError = null;

		if (!id)
		{
			var onFinishName = options['onfinish'];
			var onErrorName = options['onerror'];
			id = Math.floor(Math.random() * (1000000000+1));
			oObj.setAttribute("grabzit-id",id);
			functionNameFinish = 'grabzitFinish'+id;
			functionNameError = 'grabzitError'+id;

			window[functionNameFinish] = function (id) {
			...
			window[functionNameError] = function (message, code) {
				var links = document.getElementsByClassName("grabzit-pdf-save-progress");
		...
		clonedOptions['onfinish'] = functionNameFinish;
		clonedOptions['onerror'] = functionNameError;
