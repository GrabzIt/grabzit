...
if(url==null)
{url=location.href}
var functionName=null;if(!id)
{id=Math.floor(Math.random()*(1000000000+1));oObj.setAttribute("grabzit-id",id);functionName='grabzit'+id;window[functionName]=function(){var links=document.getElementsByClassName("grabzit-pdf-save-progress");for(var i=0;i<links.length;i++)
{var link=links[i];if(link.getAttribute("grabzit-id")==id)
{link.className=link.className.replace(/\b grabzit-pdf-save-progress\b/,'')}}}}
functionName='grabzit'+id;var clonedOptions=JSON.parse(JSON.stringify(options));clonedOptions.onfinish=functionName;clonedOptions.onerror=functionName;try
...
if(options!=null)
{options.format='pdf';options.download=1}
