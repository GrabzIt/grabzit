...
{this.options=options;this.key=key;this.pluuginId=null;this.clicked=function(oObj)
{if(oObj==null||oObj.className.indexOf("grabzit-pdf-save-progress")!==-1)
{return!1}
oObj.className+=" grabzit-pdf-save-progress";var id=oObj.getAttribute("grabzit-id");var url=null;if(oObj.getAttribute("grabzit-url")!=null)
{url=oObj.getAttribute("grabzit-url")}
if(url==null)
{url=location.href}
var functionNameFinish=null;var functionNameError=null;if(!id)
{var onFinishName=options.onfinish;var onErrorName=options.onerror;id=Math.floor(Math.random()*(1000000000+1));oObj.setAttribute("grabzit-id",id);functionNameFinish='grabzitFinish'+id;functionNameError='grabzitError'+id;window[functionNameFinish]=function(id){var links=document.getElementsByClassName("grabzit-pdf-save-progress");for(var i=0;i<links.length;i++)
{var link=links[i];if(link.getAttribute("grabzit-id")==id)
{link.className=link.className.replace(/\b grabzit-pdf-save-progress\b/,'')}}
if(onFinishName!=null)
{var finishFunc=new Function(onFinishName+"('"+id+"')");finishFunc()}}
window[functionNameError]=function(message,code){var links=document.getElementsByClassName("grabzit-pdf-save-progress");for(var i=0;i<links.length;i++)
{var link=links[i];if(link.getAttribute("grabzit-id")==id)
{link.className=link.className.replace(/\b grabzit-pdf-save-progress\b/,'')}}
if(onErrorName!=null)
{var errorFunc=new Function(onErrorName+"('"+message+"',"+code+")");errorFunc()}}}
functionNameFinish='grabzitFinish'+id;functionNameError='grabzitError'+id;var clonedOptions=JSON.parse(JSON.stringify(options));clonedOptions.onfinish=functionNameFinish;clonedOptions.onerror=functionNameError;try
...
{that.pluginId='grabzItSaveToPDF'+Math.floor(Math.random()*(1000000000+1));if(options==null)
{options={};options.filename=document.title+'.pdf'}
if(options!=null)
{options.format='pdf';options.download=1;options=GrabzIt()._cleanOptions(options)}
var links=document.getElementsByClassName("grabzit-pdf-save");for(var i=0;i<links.length;i++)
{var link=links[i];var eventVal='';if(link.getAttribute('onclick')!=null)
...
window[that.pluginId]=that}(this)}